...
def main():
    ...
    p.add_argument("--save", type=str, default="outputs/mobile2d_log_nln.gif")
    args = p.parse_args()
    ...
    cfg = MPPIConfig(
        ...
        gamma=args.gamma,
        u_clip=None,              # v, omega는 dynamics에서 각각 클립
        device=args.device,
        dtype=torch.float32,
    ...
    for t in range(T):
        ...
        if args.record_sample:
            ...
            if t == 0:
                np.save("outputs/sample_noise_log_nln.npy", noise[:,0,:].detach().cpu().numpy())
                np.save("outputs/sample_traj_log_nln.npy", Xs[:,:,:2].detach().cpu().numpy())
        else:
            u = ctrl.step(x)
